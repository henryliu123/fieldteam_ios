(function(){
        ...
        var job_base_invoice_signature_page_obj = null;

        function job_base_invoice_signature_page(){
                var self = this;
                var window_source = 'job_base_invoice_signature_page';
                if(win.action_for_signature == 'edit_job_invoice_signature'){
                        win.title = 'Signature';
                }else{
                ...
                self.init = function(){
                        try{
                                ...
                                if(win.action_for_signature == 'edit_job_invoice_signature'){
                                        if(win.job_invoice_signature_id > 1000000000){
                ...
                self.init_vars = function(){
                        try{
                                if(win.action_for_signature == 'edit_job_invoice_signature'){
                                        ...
                                        var rows = db.execute('SELECT * FROM my_'+_type+'_invoice_signature WHERE id=? ',_selected_job_invoice_signature_id);
                                        if(rows.isValidRow()){
...
//                                        }                 
                                ...
                                if(_check_if_exist_change()){
                                        var db = Titanium.Database.open(self.get_db_name());
                                        var d = new Date();
                                        if(_selected_job_invoice_signature_id > 0){//update
                                                db.execute('UPDATE my_'+_type+'_invoice_signature SET client_name=?,changed=? '+
                                                        ' WHERE id='+_selected_job_invoice_signature_id,
                                                        _client_name,
                                                        1);
                                        }else{//add
                                                ...
                                                var file_folder = _selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_signature';
                                                ...
                                                var new_time_value = _selected_job_invoice_signature_id = parseInt(d.getTime(), 10);
                                                db.execute('INSERT INTO my_'+_type+'_invoice_signature (id,local_id,'+_type+'_id,'+((_type == 'job')?'job_invoice_id,':'quote_print_id,')+
                ...
                var _selected_job_invoice_signature_id = win.job_invoice_signature_id;
                var _client_name = '';
                var _path = '';
                ...
                function _init_signature_area_field(){
                        try{                             
                                if(win.action_for_signature == 'edit_job_invoice_signature'){
                                        ...
                                        var _file_folder = self.file_directory+_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_signature';                                        
                                                                        ...
                                                                        xhr.onload = function(){
                                                                                try{
                                                                                        if((xhr.readyState === 4)&&(this.status === 200)){                                                                                     
                                                                                                if(!self.create_directory(_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_signature')){
                                                                                                        self.hide_indicator();
                                                                                                        self.show_message(L('message_failure_create_folder'));               
                                                                                                        return;
                                                                                                }
                        ...
                        try{
                                ...
                                delete_button.addEventListener('click',function(){
                                        ...
                                        optionDialog.addEventListener('click',function(e){
                                                if(e.index === 0){
                                                        var db = Titanium.Database.open(self.get_db_name());
                                                        if(_selected_job_invoice_signature_id > 1000000000){
                                                                db.execute('DELETE FROM my_'+_type+'_invoice_signature WHERE id=?',_selected_job_invoice_signature_id);
                                                        }else{
                                                                db.execute('UPDATE my_'+_type+'_invoice_signature SET changed=?,status_code=?  WHERE id='+_selected_job_invoice_signature_id,
                                                                        1,2
                                                                        );
                                                        }
                                                        db.close();
                ...
                function _upload_signature(){
                        try{
                                ...
                                var status_code =0;                                
                                var db = Titanium.Database.open(self.get_db_name());
                                var sql = 'select * from my_'+_type+'_invoice_signature where id=?';
                                var rows = db.execute(sql,_selected_job_invoice_signature_id);                                
                                if(rows.isValidRow()){
                                ...
                                xhr.onload = function(){
                                        try{
                                                ...
                                                if((xhr.readyState === 4)&&(this.status === 200)){
                                                        ...
                                                        if(_type == 'job'){
                                                                self.update_selected_tables(null,'JobInvoiceSignature',result); 
                                                        }else{
                                                                self.update_selected_tables(null,'QuoteInvoiceSignature',result); 
                                                        }
                                ...
                                xhr.send({
                                        ...
                                        'job_invoice_id':_selected_job_invoice_id,
                                        'quote_print_id':_selected_job_invoice_id,
                ...
                function _upload_signature_image_file(local_id){
                        try{
                                ...
                                var temp_row = db.execute('SELECT * FROM my_'+_type+'_invoice_signature '+' WHERE local_id=?',local_id);
                                ...
                                if(temp_flag){
                                        ...
                                        xhr.send({
                                                ...
                                                'table_name':'my_'+_type+'_invoice_signature',
                                                'width':temp_width,
                                                'height':temp_height,
                ...
                function _download_invoice_file(){
                        try{
                                ...
                                xhr.onload = function(){
                                        try{                                                                             
                                                if((xhr.readyState === 4)&&(this.status === 200)){                                                           
                                                        if(!self.create_directory(_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice')){
                                                                self.hide_indicator();
                                                                self.show_message(L('message_failure_create_folder'));               
                                                                return;
                                                        }
                                                        var db = Titanium.Database.open(self.get_db_name());
                                                        var rows = db.execute('SELECT * FROM my_'+_type+'_invoice where id=?',_selected_job_invoice_id);
                                                        ...
                                                        if(invoice_file_path != ''){
                                                                ...
                                                                var _file_name = _file_name_array[_file_name_array.length-1];
                                                                var _file_folder = self.file_directory+_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice';
                                                                var media_obj = Ti.Filesystem.getFile(_file_folder,_file_name);
                                                                media_obj.write(this.responseData);
                                                                Ti.App.fireEvent('refresh_invoice_file'); 
                                                        }
                                                        self.hide_indicator();
                                                        win.close();
                                                }else{
        ...
        win.addEventListener('open',function(){
                try{
                        if(job_base_invoice_signature_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                job_base_invoice_signature_page.prototype = new F();
                                job_base_invoice_signature_page.prototype.constructor = job_base_invoice_signature_page;
                                job_base_invoice_signature_page_obj = new job_base_invoice_signature_page();
                                job_base_invoice_signature_page_obj.init();
                        }
                        job_base_invoice_signature_page_obj.display();
                }catch(err){
                        alert(err);
                        return;
                }
        ...
        win.addEventListener('close',function(){
                try{
                        job_base_invoice_signature_page_obj.close_window();
                        job_base_invoice_signature_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
