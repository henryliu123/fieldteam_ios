...
(function(){
        ...
        var base_job_address_page_obj = null;

        function base_job_address_page(){
                var self = this;
                var window_source = 'base_job_address_page';
                win.title = self.ucfirst(win.type)+' Address';
                ...
                self.init_vars = function(){
                        try{
                                if(_sub_number != ''){
                                        _sub_number = _sub_number.toUpperCase();
                                }
                                if(_unit_number != ''){
                                        _unit_number = _unit_number.toUpperCase();
                                }
                                if(_street_number != ''){
                                        _street_number = _street_number.toUpperCase();
                                }        
                                if(_street != ''){
                                        _street = _street.toUpperCase();
                                }
                                if(_suburb != ''){
                                        _suburb = _suburb.toUpperCase();
                                }
                                if(_postcode != ''){
...
//                                }
				...
				if(self.trim(_street) == ''){
					if(self.trim(_suburb) == ''){
						...
						new_addr = '';
                                ...
                                Ti.API.info('new_addr_state_and_country:'+new_addr_state_and_country);
                                if(Ti.Network.online){
                                        ...
                                        _save_address_to_database(latitudeText,longitudeText);
                                        if(e.index == self.default_main_menu_button_index){//menu menu
                ...
                self.save_address = function(params){
                        try{
                                ...
                                _street_type_id = (params.street_type_id === undefined)?0:params.street_type_id;
                                _suburb_id = (params.suburb_id === undefined)?0:params.suburb_id;
                                _postcode = (params.postcode === undefined)?'':params.postcode;
                                _suburb = (params.suburb === undefined)?'':params.suburb;
                                _save_address_to_database(params.latitude,params.longitude);
                ...
                self.update_field_value_after_set_properites = function(){
                        try{                                                                                                
                                if(Ti.App.Properties.getBool('update_unique_code_with_search_job_address_view_flag')){                                      
                                        _stateID = Ti.App.Properties.getString('update_unique_code_with_search_job_address_view_id');
                                        _state = Ti.App.Properties.getString('update_unique_code_with_search_job_address_view_content');
                                        if(_current_selected_row != null){
                                                _current_selected_row.children[_current_selected_row.children.length-1].text = _state;                                                       
                                        }                                      
                                        Ti.App.Properties.setBool('update_unique_code_with_search_job_address_view_flag',false);
                                }    
                ...
                var _stateID = (win.stateID === 0)?2:win.stateID;                
                var _state_abbr='';
                ...
                function _event_for_state(e){
                        try{
                                ...
                                var client_win = Ti.UI.createWindow({
                                        ...
                                        query_array:['status_code=1','locality_country_id='+country_id],//query field
                 ...
                 *  update address info
                 */                
                function _save_address_to_database(latitudeText,longitudeText){
                        try{
                                if(self.trim(_street) != ''){
                                        var temp_array = _street.split(' ');
                                ...
                                var db = Titanium.Database.open(self.get_db_name());
                                db.execute('UPDATE my_'+_type+' SET sub_number=?,unit_number=?,street_number=?,locality_street_type_id=?,street=?,street_type=?,locality_suburb_id=?,suburb=?,postcode=?,locality_state_id=?,latitude=?,longitude=?,changed=1 WHERE id=?',
                                        _sub_number,
                                        _unit_number,
                                        _street_number,
                                        _street_type_id,
                                ...
                                self.process_simple_error_message(err,window_source+' - _save_address_to_database');
                                return; 
                        }                                
                }
                /**
        ...
        win.addEventListener('focus',function(){
                try{
                        if(base_job_address_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                base_job_address_page.prototype = new F();
                                base_job_address_page.prototype.constructor = base_job_address_page;
                                base_job_address_page_obj = new base_job_address_page();
                                base_job_address_page_obj.init();
                        }else{
                                base_job_address_page_obj.update_field_value_after_set_properites(); 
                        }
                }catch(err){
                        alert(err);
                        return;
        ...
        win.addEventListener('close',function(){
                try{
                        base_job_address_page_obj.close_window();
                        base_job_address_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
