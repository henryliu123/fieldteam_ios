...
(function(){
        ...
        var job_client_contact_list_page_obj = null;

        function job_client_contact_list_page(){
                var self = this;
                var window_source = 'job_client_contact_list_page';
                        ...
                        try{
                                ...
                                if(rows.getRowCount() > 0){
                                        while(rows.isValidRow()){
                                                var client_contact_row = db.execute('SELECT * FROM my_client_contact WHERE id=? and status_code=1',rows.fieldByName('client_contact_id'));
                                                if(client_contact_row.isValidRow()){
                                                        ...
                                                        var edit_btn = Ti.UI.createButton({
                                                                ...
                                                                left:10,
                                                                top:((client_contact_row.fieldByName('position') != null)&&(self.trim(client_contact_row.fieldByName('position')) != ''))?5:'auto',
                                                                width:_header_view_button_width-5,
                                                                height:_header_view_button_height
                                                        });
                                                        row.add(edit_btn);                                                

                                                        if((client_contact_row.fieldByName('position') != null)&&(self.trim(client_contact_row.fieldByName('position')) != '')){
                                                                var title_label = Ti.UI.createLabel({
                                                                        top:5,
                                                                        left:10+_header_view_button_width,
                                                                        width:self.screen_width-100,
                                                                        text:client_contact_row.fieldByName('first_name')+' '+client_contact_row.fieldByName('last_name'),
                                                                        height:20,
                                                                        font:{
                                                                ...
                                                                var position_label = Ti.UI.createLabel({
                                                                        ...
                                                                        left:10,
                                                                        width:self.screen_width-100,
                                                                        text:'['+client_contact_row.fieldByName('position')+']',
                                                                        height:20,
                                                                        font:{
                ...
                function _event_for_delete_btn(e){
                        try{
                                var job_client_contact_id = e.row.job_client_contact_id;
                                var db = Titanium.Database.open(self.get_db_name());
                                if(job_client_contact_id > 1000000000){
                                        db.execute('DELETE FROM my_'+_type+'_client_contact WHERE id=?',job_client_contact_id);
                                }else{
                                        db.execute('UPDATE my_'+_type+'_client_contact SET changed=?,status_code=?  WHERE id='+job_client_contact_id,
                                                1,2
                                                );
                                }
                                db.close();
        ...
        win.addEventListener('focus',function(){
                try{
                        if(job_client_contact_list_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                job_client_contact_list_page.prototype = new F();
                                job_client_contact_list_page.prototype.constructor = job_client_contact_list_page;
                                job_client_contact_list_page_obj = new job_client_contact_list_page();
                                job_client_contact_list_page_obj.init();
                        }else{
                                job_client_contact_list_page_obj.display();
                        }
                }catch(err){
                        alert(err);
                        return;
        ...
        win.addEventListener('close',function(){
                try{
                        job_client_contact_list_page_obj.close_window();
                        job_client_contact_list_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
