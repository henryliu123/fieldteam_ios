...
(function(){
        ...
        var job_title_page_obj = null;

        function job_title_page(){
                var self = this;
                var window_source = 'job_title_page';
                win.title = 'Edit Title';
                ...
                self.init = function(){
                        try{        
                                ...
                                if(!self.is_ios_7_plus()){
                                        ...
                                        _init_job_title_field_for_ios7();
                                }                                                              
                        }catch(err){
                ...
                self.nav_right_btn_click_event = function(e){
                        try{
                                if(_check_if_exist_change()){
                                        if(self.trim(_title).length <=0){
                                                self.show_message(L('message_enter_title_in_job_title'));
                                        ...
                                        if(_selected_job_id > 0){//update
                                                db.execute('UPDATE my_'+_type+' SET title=?,changed=? '+
                ...
                function _init_job_title_field(){
                        try{
                                var title_field = Ti.UI.createTableViewRow({
                                        className:'set_job_title',
                                        filter_class:'set_job_title',
                                        height:'auto'
                                });
                                self.job_title_content_field = Ti.UI.createTextArea({
                                ...
                                title_field.add(self.job_title_content_field);
                                self.data.push(title_field);
                                self.job_title_content_field.addEventListener('change',function(e){
                                        _title = e.value;
                                });
                        }catch(err){
                ...
                function _init_job_title_field_for_ios7(){
                        try{
                                self.job_title_content_field = Ti.UI.createTextArea({
                                ...
                                win.add(self.job_title_content_field);
                                self.job_title_content_field.addEventListener('change',function(e){
                                        _title = e.value;
                                });
                        }catch(err){
                                self.process_simple_error_message(err,window_source+' - _init_job_title_field_for_ios7');
                                return;
                        }
                }                       
                /**
                ...
                function _check_if_exist_change(){
                        try{
                                ...
                                var rows = db.execute('SELECT * FROM my_'+_type+' WHERE id=?',_selected_job_id);
                                if((rows.getRowCount() > 0) && (rows.isValidRow())){
                                        if(_title != rows.fieldByName('title')){
        ...
        win.addEventListener('open',function(){
                try{
                        if(job_title_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                job_title_page.prototype = new F();
                                job_title_page.prototype.constructor = job_title_page;
                                job_title_page_obj = new job_title_page();
                                job_title_page_obj.init();
                                job_title_page_obj.job_title_content_field.focus();
                        }
                }catch(err){
                        alert(err);
                        return;
        ...
        win.addEventListener('close',function(){
                try{
                        job_title_page_obj.close_window();
                        job_title_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
