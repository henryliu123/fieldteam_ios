(function(){
        ...
        var job_base_invoice_locksmith_signature_page_obj = null;

        function job_base_invoice_locksmith_signature_page(){
                var self = this;
                var window_source = 'job_base_invoice_locksmith_signature_page';
                if(win.action_for_signature == 'edit_job_invoice_locksmith_signature'){
                        win.title = 'Signature';
                }else{
                ...
                self.init = function(){
                        try{
                                ...
                                if(win.action_for_signature == 'edit_job_invoice_locksmith_signature'){
                                        if(win.job_invoice_locksmith_signature_id > 1000000000){
                ...
                self.init_vars = function(){
                        try{
                                if(win.action_for_signature == 'edit_job_invoice_locksmith_signature'){
                                        ...
                                        var rows = db.execute('SELECT * FROM my_'+_type+'_invoice_locksmith_signature WHERE id=? ',_selected_job_invoice_locksmith_signature_id);
                                        if(rows.isValidRow()){
...
//                                        }                 
                                ...
                                if(_check_if_exist_change()){
                                        var db = Titanium.Database.open(self.get_db_name());
                                        var d = new Date();
                                        if(_selected_job_invoice_locksmith_signature_id > 0){//update
                                                db.execute('UPDATE my_'+_type+'_invoice_locksmith_signature SET client_name=?,changed=? '+
                                                        ' WHERE id='+_selected_job_invoice_locksmith_signature_id,
                                                        _client_name,
                                                        1);
                                        }else{//add
                                                ...
                                                var file_folder = _selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_locksmith_signature';
                                                ...
                                                var new_time_value = _selected_job_invoice_locksmith_signature_id = parseInt(d.getTime(), 10);
                                                db.execute('INSERT INTO my_'+_type+'_invoice_locksmith_signature (id,local_id,'+_type+'_id,'+((_type == 'job')?'job_invoice_locksmith_id,':'quote_print_locksmith_id,')+
                                                        ...
                                                        _selected_job_id,
                                                        _selected_job_invoice_locksmith_id,
                                                        _client_name,
                ...
                var _selected_job_invoice_locksmith_id = win.job_invoice_locksmith_id;
                var _selected_job_invoice_locksmith_signature_id = win.job_invoice_locksmith_signature_id;
                var _client_name = '';
                var _path = '';
                ...
                function _init_signature_area_field(){
                        try{                             
                                if(win.action_for_signature == 'edit_job_invoice_locksmith_signature'){
                                        ...
                                        var _file_folder = self.file_directory+_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_locksmith_signature';                                        
                                                                        ...
                                                                        xhr.onload = function(){
                                                                                try{
                                                                                        if((xhr.readyState === 4)&&(this.status === 200)){                                                                                     
                                                                                                if(!self.create_directory(_selected_company_id+'/'+_type+'/'+_selected_job_reference_number+'/invoice_locksmith_signature')){
                                                                                                        self.hide_indicator();
                                                                                                        self.show_message(L('message_failure_create_folder'));               
                                                                                                        return;
                                                                                                }
                        ...
                        try{
                                ...
                                delete_button.addEventListener('click',function(){
                                        ...
                                        optionDialog.addEventListener('click',function(e){
                                                if(e.index === 0){
                                                        var db = Titanium.Database.open(self.get_db_name());
                                                        if(_selected_job_invoice_locksmith_signature_id > 1000000000){
                                                                db.execute('DELETE FROM my_'+_type+'_invoice_locksmith_signature WHERE id=?',_selected_job_invoice_locksmith_signature_id);
                                                        }else{
                                                                db.execute('UPDATE my_'+_type+'_invoice_locksmith_signature SET changed=?,status_code=?  WHERE id='+_selected_job_invoice_locksmith_signature_id,
                                                                        1,2
                                                                        );
                                                        }
                                                        db.close();
                ...
                function _upload_signature(){
                        try{
                                ...
                                var status_code =0;                                
                                var db = Titanium.Database.open(self.get_db_name());
                                var sql = 'select * from my_'+_type+'_invoice_locksmith_signature where id=?';
                                var rows = db.execute(sql,_selected_job_invoice_locksmith_signature_id);                                
                                if(rows.isValidRow()){
                                ...
                                xhr.onload = function(){
                                        try{
                                                if((xhr.readyState === 4)&&(this.status === 200)){
                                                        ...
                                                        if(_type == 'job'){
                                                                self.update_selected_tables(null,'JobInvoiceLocksmithSignature',result); 
                                                        }else{
                                                                self.update_selected_tables(null,'QuoteInvoiceLocksmithSignature',result); 
                                                        }
                                ...
                                xhr.send({
                                        'type':'upload_invoice_locksmith_signature',
                                        ...
                                        'job_invoice_locksmith_id':_selected_job_invoice_locksmith_id,
                                        'quote_print_locksmith_id':_selected_job_invoice_locksmith_id,
                ...
                function _upload_signature_image_file(local_id){
                        try{
                                ...
                                var temp_row = db.execute('SELECT * FROM my_'+_type+'_invoice_locksmith_signature '+' WHERE local_id=?',local_id);
                                ...
                                if(temp_flag){
                                        ...
                                        xhr.send({
                                                ...
                                                'table_name':'my_'+_type+'_invoice_locksmith_signature',
                                                'width':temp_width,
                                                'height':temp_height,
                ...
                function _download_invoice_file(){
                        try{
                                self.update_message('Downloading Invoice...',true);
                                var xhr = null;
                                if(self.set_enable_keep_alive){
                                        xhr = Ti.Network.createHTTPClient({
                                                enableKeepAlive:false
                                ...
                                xhr.onload = function(){
                                        try{                                                                             
                                                if((xhr.readyState === 4)&&(this.status === 200)){                                                        
                                                        ...
                                                        var db = Titanium.Database.open(self.get_db_name());
                                                        var rows = db.execute('SELECT * FROM my_'+_type+'_invoice_locksmith where id=?',_selected_job_invoice_locksmith_id);
                                                        var invoice_file_path = '';
                                                        if(rows.isValidRow()){
                                                                invoice_file_path = rows.fieldByName('path');
                                                        }
                                                        rows.close();                                                        
                                                        db.close();
                                                        if(invoice_file_path != ''){
                                                                var _file_name_array = invoice_file_path.split('/');
                                                                var _file_name = _file_name_array[_file_name_array.length-1];
                                                                ...
                                                                Ti.App.fireEvent('refresh_invoice_locksmith_file'); 
                                                        }
                                                        self.hide_indicator();
                                                        win.close();
                                                }else{
        ...
        win.addEventListener('open',function(){
                try{
                        if(job_base_invoice_locksmith_signature_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                job_base_invoice_locksmith_signature_page.prototype = new F();
                                job_base_invoice_locksmith_signature_page.prototype.constructor = job_base_invoice_locksmith_signature_page;
                                job_base_invoice_locksmith_signature_page_obj = new job_base_invoice_locksmith_signature_page();
                                job_base_invoice_locksmith_signature_page_obj.init();
                        }
                        job_base_invoice_locksmith_signature_page_obj.display();
                }catch(err){
                        alert(err);
                        return;
                }
        ...
        win.addEventListener('close',function(){
                try{
                        job_base_invoice_locksmith_signature_page_obj.close_window();
                        job_base_invoice_locksmith_signature_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
