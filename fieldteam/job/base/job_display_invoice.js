...
(function(){
        ...
        var job_base_display_invoice_page_obj = null;

        function job_base_display_invoice_page(){
                var self = this;
                var window_source = 'job_base_display_invoice_page';
                win.title = (win.type == 'job')?'Invoice':'Quote';
                ...
                self.init = function(){
                        try{          
                                ...
                                _display();
                                Ti.App.addEventListener('refresh_invoice_file', function(){
                                        _play_pdf();
                                });                                
                        }catch(err){
                ...
                self.nav_right_btn_click_event = function(e){
                        try{
                                ...
                                var sql = '';
                                if(_type == 'job'){
                                        sql = 'select id from my_'+_type+'_invoice_signature where job_invoice_id=? and status_code=1';
                                }else{
                                        sql = 'select id from my_'+_type+'_invoice_signature where quote_print_id=? and status_code=1';
                                }
                                var rows = db.execute(sql,_invoice_id);
                                signatureCount = rows.getRowCount();
                                _signature_id = rows.fieldByName('id');
                                rows.close();
                                                                
                                //check is_merge_signature field
                                rows = db.execute('select * from my_'+_type+'_invoice where id=?',_invoice_id);
                                if(rows.isValidRow()){
                                                ...
                                                switch(evt.index){
                                                        ...
                                                        case 1://add signature or display signature
                                                                var new_win = Ti.UI.createWindow({
                                                                        url:self.get_file_path('url','job/base/job_invoice_signature.js'),
                                                                        type:_type,
                                                                        job_id:_selected_job_id,
                                                                        job_reference_number:_selected_job_reference_number,
                                                                        job_invoice_id:_invoice_id,
                                                                        job_invoice_signature_id:_signature_id,
                                                                        action_for_signature:(_signature_id>0)?'edit_job_invoice_signature':'add_job_invoice_signature'
                                                                });
                ...
                var _invoice_id = win.invoice_id;
                var _signature_id = 0;
                 ...
                 */
                function _send_email_for_invoce(){
                        try{
                                var array = [];
                                var db = Titanium.Database.open(self.get_db_name());
                                var rows = db.execute('SELECT * FROM my_'+_type+'_invoice WHERE id=?',_invoice_id);
                                ...
                                self.process_simple_error_message(err,window_source+' - _send_email_for_invoce');
                                return;
                        }                                    
                }
                /*
        ...
        win.addEventListener('open',function(){
                try{
                        if(job_base_display_invoice_page_obj === null){
                                var F = function(){};
                                F.prototype = FieldTeam.prototype;
                                job_base_display_invoice_page.prototype = new F();
                                job_base_display_invoice_page.prototype.constructor = job_base_display_invoice_page;
                                job_base_display_invoice_page_obj = new job_base_display_invoice_page();
                                job_base_display_invoice_page_obj.init();
                        }
                }catch(err){
                        alert(err);
                        return;
        ...
        win.addEventListener('close',function(){
                try{
                        job_base_display_invoice_page_obj.close_window();
                        job_base_display_invoice_page_obj = null;
                        win = null;
                }catch(err){
                        alert(err);
                        return;
